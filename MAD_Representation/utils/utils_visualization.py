...
class Visualization:
    ...
    def __init__(self, kwargs, eval_id=-1):
        self.img_idx = 0
        self.px = 512
        self.color_scheme = "gray"  # gray / blue_red / green_red
    ...
    def update(self, inputs, flow, iwe, brightness,flow_feature_map=False):
        """
        Live visualization.
        ...
        # cv2.resizeWindow("Input Events", (720,360))
        cv2.imshow("Input Events", self.events_to_image(inp_events_npy))

        # if flow_feature_map is not None:
        #     feature_map = flow_feature_map[0, 0, :, :]
        #     plt.imshow(feature_map, cmap='jet', vmin=feature_map.min(), vmax=feature_map.max())
        #     plt.show()
        ...
        # optical flow
        fig = plt.figure('img',figsize=(10,5))
        if flow is not None:
            flow = flow.detach()
            flow_npy = flow.cpu().numpy().transpose(0, 2, 3, 1).reshape((height, width, 2))
        #
        #     flow_mask = flow_npy ** 2).sum(axis=2)
        #     kernel = cv2.getGaussianKernel(7,sigma=1)
        #     flow_mask_dial = cv2.dilate(flow_mask,kernel,iterations=1)
        #     ax1 = fig.add_subplot(1,2,1)
        #     ax2 = fig.add_subplot(1, 2, 2)
        #     ax1.imshow(flow_mask, cmap='jet', vmin=0, vmax=0.002)
        #     ax2.imshow(flow_mask_dial, cmap='jet', vmin=0, vmax=0.002)
        #     plt.pause(0.01)
        #     fig.clf()
    ...
    @staticmethod
    def events_to_image(inp_events, color_scheme="gray"):
        """
        Visualize the input events.
        :param inp_events: [ H x W x 2] per-pixel and per-polarity event count
        :param color_scheme: green_red/gray
        :return event_image: [H x W x 3] color-coded event image
        """
        pos = inp_events[:, :, 0]#h,w,2
        neg = inp_events[:, :, 1]
    ...
    @staticmethod
    def events_to_image3(inp_events,in_max, color_scheme="gray"):
        """
        Visualize the input events.
        :param inp_events: [ H x W x 2] per-pixel and per-polarity event count
        :param color_scheme: green_red/gray
    ...
    @staticmethod
    def events_to_image2(inp_events, color_scheme="gray"):
        """
        Visualize the input events.
        :param inp_events: [ H x W x 2] per-pixel and per-polarity event count
        :param color_scheme: green_red/gray
