...
class EVFlowNet(BaseModel):
    ...
    def __init__(self, unet_kwargs, num_bins):
        super().__init__()
        self.crop = None
        self.mask = unet_kwargs["mask_output"] # False
        EVFlowNet_kwargs = {
            "base_num_channels": unet_kwargs["base_num_channels"],#32
            "num_encoders": 4,
            "num_residual_blocks": 2,
    ...
    def forward(self, inp_voxel, inp_cnt):
        ...
        # print(multires_flow[0].shape)
        # print(multires_flow[1].shape)
        # print(multires_flow[2].shape)
        # print(multires_flow[3].shape)
        ...
        for flow in multires_flow:
            ...
            # print('scale:', multires_flow[-1].shape[2] / flow.shape[2],multires_flow[-1].shape[3] / flow.shape[3],)
