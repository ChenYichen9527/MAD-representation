...
class EventWarping(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        self.res = config["train"]["resize"] #for 1MPX DATASET
        self.flow_scaling = max(config["train"]["resize"])
        # self.res = config["dataset"]["shape"]
        # self.flow_scaling = max(config["dataset"]["shape"])
        self.weight = 0.5
        self.device = device
    ...
    def forward(self, flow_list, event_list, pol_mask):
        ...
        :param event_list: [batch_size x N x 4] input events (y, x, ts, p)  (ts,x,y,p)?
        ...
        for flow in flow_list:
            ...
            devive='cuda:0'
            fw_idx = fw_idx.to(devive)
            fw_weights = fw_idx.to(devive)
            ts_list = ts_list.to(devive)
            ...
            bw_idx = bw_idx.to(devive)
            bw_weights = bw_weights.to(devive)
            ...
            flow_dx = torch.sqrt(flow_dx ** 2 + 1e-6)
            flow_dy = torch.sqrt(flow_dy ** 2 + 1e-6)
